...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Fix paths for local development (if not at root)
  // This is a simple path adjuster based on current URL depth
  const depth = window.location.pathname.split('/').length - 2;
  const prefix = depth > 0 ? '../'.repeat(depth) : './';
  
  if (depth > 0 && !window.location.hostname) {
    // We are on a subpage locally
